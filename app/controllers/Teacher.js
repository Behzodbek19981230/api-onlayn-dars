...
class TeacherController {
  ...
  static async add(req, res) {
    try {
      ...
      const banner = await Teacher.create({
  ...
  static async edit(req, res) {
    try {
      const bannerFind = await Teacher.findByPk(req.params.id);
      ...
      const banner = await Teacher.update(data, {
  ...
  static async delete(req, res) {
    try {
      const response = await Teacher.findByPk(req.params.id);

      if (!response) {
        return errorResponse(res, 404, "Not found", "Cannot find food");
      }
      const destroy = await Teacher.destroy({ where: { id: req.params.id } });
